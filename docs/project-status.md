...
- ✅ עמוד בית + חיפוש אמיתי (אזור + אורחים), עם **לוח שנה יחיד לבחירת טווח
  תאריכים** (`react-day-picker`, זרימת קליק-קליק כמו הזמנת טיסה) שמסמן חגים
  ומועדים יהודיים על גבי הלוח (מחושב מקומית עם `@hebcal/core`, בלי קריאת
  רשת) — עובד גם ב-RTL/עברית וגם ב-LTR/אנגלית. **הלוח סגור כברירת מחדל**
  (שדה קטן, כמו באתרי הזמנות) ונפתח כ-popover בלחיצה; נסגר אוטומטית אחרי
  שנבחר טווח מלא, או בלחיצה מחוץ ללוח / `Escape`
...
| עיצוב חזותי/מותג | **שלב 1 (דף הבית בלבד) בוצע** ב-22 בספטמבר 2026 לפי תדמית שסופקה
  (`docs/handoff-2026-09-22-visual-design-phase1-homepage.md`) — טוקני צבע
  (`--color-brand` וכו') ופונט serif (`Frank Ruhl Libre`) נוספו ב-
  `globals.css`/`layout.tsx`, יושמו בלוגו, בכותרת דף הבית, ובטופס החיפוש.
  **שאר האתר** (חיפוש/תוצאות, עמוד דירה, דשבורד בעלים, פאנל ניהול) עדיין
  בעיצוב הישן — ממתין לאישור בעל האתר על מראה דף הבית לפני שלב 2 |
