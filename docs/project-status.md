...
**עדכון אחרון:** 22 בספטמבר 2026 (כולל עדכון לילי)
**מטרת המסמך:** נקודת המשך ברורה — לך, ולכל שיחת Claude עתידית שתמשיך את הפרויקט הזה.
קרא את זה קודם כל, לפני שממשיכים לבנות.

---

## 0. מה נבנה בלילה (בלי אישור בזמן אמת שלך)

אמרת "לך לישון, תמשיך אתה" — אז המשכתי, אבל בלי היכולת לקבל ממך אישור שלב-אחר-שלב
כמו כל הלילה שקדם לזה. בניתי שני דברים, בסדר הזה:

1. **תחילת פאנל הניהול (שלב ו')**, ובדיוק טאב **"אישור דירות"** בלבד (לא 6 הטאבים) —
   כי זה היה הצורך הכי דחוף שהזכרת (אישור ידני ב-SQL מסורבל).
2. **ה'-5** — שכפול דירה קיימת (טופס מוכן-מראש מדירה אחרת שלך, בלי התמונות) + בורר
   מצב תפוסה (נעילה מלאה/פתוח/סגור) בדשבורד, על כל דירה בנפרד.

לא נגעתי בשאלת המפה שנשארה פתוחה.

**פעולה אחת שאתה חייב לעשות לפני שזה עובד באתר החי (Vercel):**
נוצרה סיסמה זמנית אקראית לפאנל הניהול, שמורה כרגע רק ב-`.env.local` (לא ב-Git).
צריך:
1. להחליט על סיסמה משלך (או להשתמש בזמנית: `IoFkvXUUgw5x`)
2. להוסיף אותה כמשתנה סביבה חדש ב-Vercel: `ADMIN_PASSWORD` (Project Settings ←
   Environment Variables), ל-Production **וגם** Preview
3. בלי זה, `/admin/login` באתר החי תמיד יגיד "סיסמה שגויה" (ברירת המחדל הבטוחה —
   הפאנל נעול כשאין סיסמה מוגדרת, לא פתוח בטעות)

איך זה עובד: `/admin/login` (סיסמה יחידה, לא קשור ל-Supabase Auth של האתר הציבורי)
← `/admin/properties` (טבלת כל הדירות, כפתורי אשר/דחה שמעדכנים ישירות את
`approval_status`). הכל מתועד גם ב-`docs/known-limitations.md`.

הכול נבדק כאן (build + lint + typecheck נקיים) אבל **לא נבדק על האתר החי** — אותה
מגבלת רשת כמו תמיד (לא יכולתי לגשת בעצמי ל-Vercel/Supabase). תבדוק אתה כשתתעורר
ותגיד מה קרה.
...
src/
  ...
  components/
    ...
    PropertyForm.tsx               הטופס הגדול לפרסום דירה (תומך גם בשכפול, initialProperty)
    AvailabilityModeSelect.tsx     בורר מצב תפוסה בדשבורד (קורא ל-owner/actions.ts)
  ...
  app/[locale]/
    ...
    owner/actions.ts                Server Action: עדכון availability_mode (RLS, לא service role)
  ...
  app/admin/                       פאנל ניהול - לא תחת [locale] בכוונה, לא דו-לשוני
    login/page.tsx, properties/page.tsx, actions.ts (Server Actions)
  lib/admin-auth.ts                בדיקת סיסמת מנהל (עוגייה, לא Supabase Auth)
  lib/supabase/admin.ts            לקוח Supabase עם service_role key - שרת בלבד!
```
...
- ✅ פאנל ניהול חלקי (`/admin`) — התחברות בסיסמה + טבלת דירות עם אשר/דחה
  (נבנה בלילה, **טרם נבדק על האתר החי** — צריך קודם להוסיף `ADMIN_PASSWORD` ב-Vercel,
  ראה סעיף 0)
- ✅ שכפול דירה (טופס מוכן-מראש, בלי תמונות) + בורר מצב תפוסה בדשבורד
  (נבנה בלילה, **גם זה טרם נבדק על האתר החי**)
...
| **פאנל ניהול (שלב ו')** | טאב "אישור דירות" נבנה (ראה סעיף 0) — 5 טאבים נוספים חסרים | להוסיף `ADMIN_PASSWORD` ב-Vercel, לבדוק, ואז להמשיך לתשלומים/אנליטיקס/תמיכה/בעלי דירות/שוכרים |
...
| סינון לפי תאריכים ספציפיים | חיפוש מסנן רק לפי אזור+אורחים | טבלת זמינות לפי תאריך (מצב התפוסה עצמו כן ניתן לעדכון עכשיו, ראה סעיף 0) |
...
1. **קודם כל:** תוסיף `ADMIN_PASSWORD` ב-Vercel (סעיף 0), ותבדוק:
   - `/admin/login` ו-`/admin/properties` עובדים
   - בדשבורד: כפתור "שכפל" על דירה קיימת פותח טופס מלא מראש (חוץ מתמונות)
   - בדשבורד: שינוי "מצב תפוסה" בבורר נשמר (רענון הדף מראה את הבחירה שנשמרה)
2. תגיד ל-Claude "תמשיך מהמסמך `docs/project-status.md`" (או פשוט תתאר מה אתה רוצה —
   המסמך הזה כבר בגיטהאב, כל שיחה חדשה יכולה לקרוא אותו)
3. **החלטות שמחכות לך:** מפה עכשיו או אחר כך? להמשיך עם 5 הטאבים הנוספים בפאנל הניהול?
4. תזכורת לעצמך על אופן העבודה שסיכמנו: לפני כל פעולה — הסבר קודם, ולחכות לאישור שלך
   (חוץ מהלילה הזה, לפי בקשתך המפורשת).
