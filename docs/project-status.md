...
אתר תיווך לדירות להשכרה לטווח קצר בשכונת Crown Heights בניו יורק. (בעבר האתר תוכנן
לכסות גם את "האוהל" — ראה סעיף 6, האזור הושבת ב-22 בספטמבר 2026 והאתר ממוקד כרגע
ב-Crown Heights בלבד.) בעלי דירות מפרסמים, שוכרים מחפשים ופונים
...
1. **regions** — רשימה גמישה. שתי שורות: `crown-heights`, `the-ohel`. השורה
   `the-ohel` הושבתה (`is_active = false`) ב-22 בספטמבר 2026 — לא נמחקה, רק
   אינה מוצגת יותר באתר (ראה סעיף 6). `landmark_lat/lng` **כן מלאים כעת**
   (מולאו דרך Mapbox, ראה סעיף 2).
...
src/
  ...
  lib/
    ...
    favorites.ts                   הוק לניהול "מועדפים" ב-localStorage (client, בלי חשבון)
    fonts/noto-sans-hebrew-400.woff2  פונט מוטמע ל-PDF (subset עברי בלבד)
  components/
    Header.tsx, Footer.tsx, SearchForm.tsx, PropertyCard.tsx
    FavoriteButton.tsx              לב טוגל על כרטיס/עמוד דירה (משתמש ב-lib/favorites.ts)
    FavoritesButton.tsx             כפתור "הדירות שאהבתי" + פאנל תצוגה (עמוד הבית)
  ...
  app/[locale]/
    ...
    faq, how-it-works, terms       עמודי תוכן קבועים (מקושרים מה-Footer)
    list-property/page.tsx         מנתב: מחובר→טופס, לא מחובר→login
  app/api/favorites/route.ts       GET לפי ?ids=... — דירות מאושרות בלבד, לפאנל המועדפים
...
- ✅ שלד אתר: לוגו, ניווט מלא בדסקטופ (המבורגר נשאר רק במובייל), בורר שפה, RTL/LTR,
  Footer בכל עמוד ציבורי — הלוגו כעת בפונט serif (`Frank Ruhl Libre`) ובצבע המותג
  `#6e4e37` (שלב 1 בעיצוב, ראה סעיף 6)
- ✅ עמוד בית = חיפוש + תוצאות **באותו עמוד** (`[locale]/page.tsx`, אין יותר
  ניתוב ל-`/search` — הטופס מנווט חזרה ל-`/` עם ה-query params, והעמוד קורא
  אותם בעצמו ומציג את הרשימה מייד מתחת לטופס; בכניסה ראשונה בלי פרמטרים
  מוצגות כל הדירות המאושרות של Crown Heights). שני שדות נפרדים "תאריך
  כניסה"/"תאריך יציאה" (לא שדה טווח אחד) שפותחים יחד **לוח שנה משותף**
  (`react-day-picker`, זרימת קליק-קליק כמו הזמנת טיסה) שמסמן חגים ומועדים
  יהודיים על גבי הלוח (מחושב מקומית עם `@hebcal/core`, בלי קריאת רשת) —
  עובד גם ב-RTL/עברית וגם ב-LTR/אנגלית. **הלוח סגור כברירת מחדל** (שדות
  קטנים, כמו באתרי הזמנות) ונפתח כ-popover בלחיצה; נסגר אוטומטית אחרי
  שנבחר טווח מלא, או בלחיצה מחוץ ללוח / `Escape`. `search/page.tsx` הישן
  עדיין קיים בקוד (לא נמחק) אבל אף מקום באתר לא מקשר אליו יותר
...
- ✅ **מועדפים (26/9)**: לב טוגל על כרטיס דירה ובעמוד דירה בודדת. **בלי התחברות/חשבון
  בכוונה** — רשימת המזהים נשמרת ב-`localStorage` של המבקר בלבד (מכשיר/דפדפן ספציפי,
  נעלמת אם מנקים נתוני אתר). כפתור "הדירות שאהבתי" ליד טופס החיפוש בעמוד הבית מציג
  מונה ופותח פאנל שמביא מהשרת (`/api/favorites`) מידע עדכני על הדירות המסומנות (כדי
  שלא יוצג מידע ישן אם דירה השתנתה/הוסרה מאז שסומנה)
...
| נושא | סטטוס |
|---|---|
| אזור "האוהל" | הושבת ב-22 בספטמבר 2026 (`is_active = false`, לא נמחק מה-DB). האתר ממוקד כרגע ב-Crown Heights בלבד — אין תיבת בחירת אזור בטופס החיפוש או בטופס פרסום דירה (הוסר לפי `docs/handoff-2026-09-22-remove-region-selector.md`) |
...
| עיצוב חזותי/מותג | **שלב 1 (דף הבית בלבד) בוצע** ב-22 בספטמבר 2026 לפי תדמית שסופקה
  (`docs/handoff-2026-09-22-visual-design-phase1-homepage.md`, ואז 3 תיקונים
  ב-`docs/handoff-2026-09-22-homepage-design-fixes.md` אחרי שבעל האתר בדק
  מול התדמית) — טוקני צבע (`--color-brand` וכו') ופונט serif (`Frank Ruhl
  Libre`) נוספו ב-`globals.css`/`layout.tsx`, יושמו בלוגו, בכותרת דף הבית,
  ובטופס החיפוש. **מצב כהה (dark mode) הוסר לגמרי מכל האתר במכוון** — האתר
  תמיד בהיר, גם אם מכשיר המבקר מוגדר למצב כהה (התדמית מעוצבת לרקע בהיר
  בלבד; כל ה-`dark:` classes הוסרו מהקוד, לא רק בדף הבית). שורת החיפוש
  מעוצבת כפס מעוגל רציף אחד עם כפתור אייקון (לא שדות נפרדים). ה-Header
  (משותף לכל האתר) מציג ניווט מלא בדסקטופ בלי המבורגר (המבורגר נשאר רק
  במובייל, `md:hidden`), עם כפתור "בעל דירה / כניסה" נפרד; נוסף Footer
  חדש (צור קשר/תנאי שימוש/copyright) בכל עמוד ציבורי, כולל עמוד `/terms`
  חדש. **שאר האתר** (עמוד דירה, דשבורד בעלים, פאנל ניהול) עדיין בעיצוב
  הישן — ממתין לאישור בעל האתר על מראה דף הבית לפני שלב 2 |
| חיבור דומיין `apartments770.com` לאתר עצמו | הדומיין קיים ומאומת ל-Resend, אבל לא מחובר ל-Vercel כדומיין של האתר |
| מועדפים בין מכשירים/דפדפנים | **הוחלט במפורש** לשמור ב-localStorage בלי חשבון (ראה סעיף 5) — אם ירצו בעתיד סנכרון בין מכשירים, זה ידרוש חשבון שוכר לצורך זה בלבד |
